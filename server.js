...
app.post("/connectionRequest/:senderUsername/:recieverUsername",async(req,res)=>{
    let status = 500;
    let message = "Internal server error"
    const invalid = () => {
        status = 401
    ...
    const result = await db.collection("connectionRequests").insertOne({recieverUsername,senderUsername,dateSent:new Date(),hasAccepted: false,dateAccepted:null}) 
...
app.put("/likeProfile/:username",async(req,res)=>{
    try {
    const memProfilesCol = db.collection("membersProfile");
    const username = req.params.username;

    const result = await memProfilesCol.updateOne({username: username},{$inc:{likes:1}});
...
app.put("/dislikeProfile/:username",async (req,res)=>{
    try {
    const memProfilesCol = db.collection("membersProfile");
    const username = req.params.username;
    ...
    if (result.modifiedCount === 1) {
        res.status(200).send("successfully liked profile")
    } else {
        throw new Error("Could not update number of likes")
    }
...
app.put("/UpdatemembersPersonalInfo/:username", async(req,res)=>{
   ...
   try {
    ...
    if (updates.username) {
        ...
        if (await db.collection("membersProfile").findOne({username:newUsername})) {
            invalid();
    ...
    const result = await db.collection("membersProfile").updateOne({username},{updates});
...
app.get("/connectionRequests/:username", async (req,res)=>{
    try {
        ...
        const result = await db.collection("connectionRequests").find({$or: [{recieverUsername:username},{senderUsername:username}]},{projection:{dataAccepted: 0, dateSent:0}}).toArray();
...
app.put("/acceptedConnectionRequest/:recieverUsername/:senderUsername",async (req,res)=>{
    try {
        const senderUsername = req.params.senderUsername;
        const recieverUsername = req.params.recieverUsername;
        
        const result = await db.collection("connectionRequests").updateOne({senderUsername,recieverUsername},{$set:{hasAccepted: true,dateAccepted: new Date()}})
...
app.put("/removeConnectionRequest/:recieverUsername/:senderUsername",async (req,res)=>{
     try {
        const senderUsername = req.params.senderUsername;
        const recieverUsername = req.params.recieverUsername;
        
        // const deletedResult = await db.collection("connectionRequests").deleteOne({senderUsername,recieverUsername})
